...
bool SSH::isFrameworkUpToDate() {
  ...
  QFile mFrameworkVersionFile(QProcessEnvironment::systemEnvironment().value("WEBOTS_HOME")+"/projects/robots/robotis/darwin-op/libraries/robotis-op2/robotis/version.txt");
...
int SSH::updateFramework(const QString &username, const QString &password) {
  ...
  const QString installArchive = QDir::tempPath() + "/webots_robotis_" + QString::number((int) QCoreApplication::applicationPid()) + "_update.zip";
...
int SSH::updateWrapper(const QString &username, const QString &password) {
  ...
  const QString installArchive = QDir::tempPath() + "/webots_robotis_" + QString::number((int)QCoreApplication::applicationPid()) + "_install.zip";
  ...
  if (sftp_mkdir(mSFTPChannel, ("/" + mainDirectoryName() + "/Linux/project/webots/controllers").toUtf8().data(), S_IRWXU) != 0) {
    // Delete local archive
    QFile deleteArchive(installArchive);
    if (deleteArchive.exists())
      deleteArchive.remove();
  ...
  // Move files and delete archive
  executeSSHCommand("mv /home/" + username + "/config/rc.local_original /" + mainDirectoryName() + "/Linux/project/webots/backup/rc.local_original");
  ...
  executeSSHCommand("mv /home/" + username + "/check_start_position /" + mainDirectoryName() + "/Linux/project/webots/check_start_position");
  executeSSHCommand("mv /home/" + username + "/config /" + mainDirectoryName() + "/Linux/project/webots/config");
  executeSSHCommand("mv /home/" + username + "/remote_control /" + mainDirectoryName() + "/Linux/project/webots/remote_control");
  ...
  executeSSHCommand("make " + makeVars() + " -C /" + mainDirectoryName() + "/Linux/project/webots/check_start_position -f Makefile.robotis-op2 clean");
  executeSSHCommand("make " + makeVars() + " -C /" + mainDirectoryName() + "/Linux/project/webots/remote_control -f Makefile.robotis-op2 clean");
  executeSSHCommand("make " + makeVars() + " -C /" + mainDirectoryName() + "/Linux/project/webots/transfer/lib -f Makefile clean");
  executeSSHCommand("make " + makeVars() + " -C /" + mainDirectoryName() + "/Linux/project/webots/lib -f Makefile.robotis-op2 clean");
  emit status("Installing Webots API: Compiling \"check_start_position\"");
  executeSSHCommand("make " + makeVars() + " -C /" + mainDirectoryName() + "/Linux/project/webots/check_start_position -f Makefile.robotis-op2");
  emit status("Installing Webots API: Compiling \"remote_control\"");
  executeSSHCommand("make " + makeVars() + " -C /" + mainDirectoryName() + "/Linux/project/webots/remote_control -f Makefile.robotis-op2");
  ...
  // Hook demo program
  command = "echo "+password+" | sudo -S cp /" + mainDirectoryName() + "/Linux/project/webots/config/rc.local_custom /etc/rc.local";
  executeSSHCommand(command);
  executeSSHCommand("ls /" + mainDirectoryName() + "/Linux/project/webots/default >/dev/null 2>&1 && echo 1 || echo 0", false);
  if (mStdout[0] == '0')
    executeSSHCommand("ln -s /" + mainDirectoryName() + "/Linux/project/demo/demo /" + mainDirectoryName() + "/Linux/project/webots/default");
...
bool SSH::isWrapperUpToDate() {
  ...
  QString version, versionInstalled;
  QStringList versionList, versionInstalledList;
  QFile mWrapperVersionFile(QProcessEnvironment::systemEnvironment().value("WEBOTS_HOME")+"/projects/robots/robotis/darwin-op/config/version.txt");
...
int SSH::startRemoteCompilation(const QString &IPAddress, const QString &username, const QString &password,bool makeDefaultController) {
  ...
  const QString controllerArchive(QDir::tempPath() + "/webots_robotis_" + QString::number((int)QCoreApplication::applicationPid()) + "_controller.zip");
  ...
  // Decompress remote controller files
  executeSSHCommand("unzip /" + mainDirectoryName() + "/Linux/project/webots/controllers/controller.zip -d /" + mainDirectoryName() + "/Linux/project/webots/controllers");

  // Deleting archive
  executeSSHCommand("rm /" + mainDirectoryName() + "/Linux/project/webots/controllers/controller.zip");

  // Compile controller
  QString makeClean = "make " + makeVars() + " -C /" + mainDirectoryName() + "/Linux/project/webots/controllers/" + controller + " -f Makefile.robotis-op2 clean";
  executeSSHCommand(makeClean);
  // touch source files with the time of the robot (in order to avoid Makefile problems if time of the robot and computer are not synchronized)
  QString command = "find /" + mainDirectoryName() + "/Linux/project/webots/controllers/"+controller+"/* -exec touch {} \\;";
  executeSSHCommand(command);
  QString makeController = "make " + makeVars() + " -C /" + mainDirectoryName() + "/Linux/project/webots/controllers/" + controller + " -f Makefile.robotis-op2";
  ...
  executeSSHCommand("echo "+password+" | sudo -S /" + mainDirectoryName() + "/Linux/project/webots/check_start_position/check_start_position");
  if (!mStdout.startsWith("Success")) {
    ...
    QString controllerExist = "ls /" + mainDirectoryName() + "/Linux/project/webots/controllers >/dev/null 2>&1 && echo 1 || echo 0" +
    ...
    if (mStdout[0] == '1') { // controller exist
      if (makeDefaultController) { // setup as default controller
        ...
        QString softLinkCommand = "cp /" + mainDirectoryName() + "/Linux/project/webots/controllers/" + controller + "/" + controller + " /" + mainDirectoryName() + "/Linux/project/webots/default";
        executeSSHCommand(softLinkCommand);
      }
      // Start controller
      executeSSHCommand("mv /" + mainDirectoryName() + "/Linux/project/webots/controllers/" + controller + "/" + controller + " /" + mainDirectoryName() + "/Linux/project/webots/controllers/" + controller + "/controller");
      executeSSHCommand("echo -e \'#!/bin/bash\\nexport DISPLAY=:0\\n/" + mainDirectoryName() + "/Linux/project/webots/controllers/"+controller+"/controller\\n\' > /" + mainDirectoryName() + "/Linux/project/webots/controllers/"+controller+"/"+controller);
      executeSSHCommand("chmod a+x /" + mainDirectoryName() + "/Linux/project/webots/controllers/"+controller+"/"+controller);
      executeSSHCommand("echo " + password + " | sudo -S /" + mainDirectoryName() + "/Linux/project/webots/controllers/"+controller+"/"+controller, true, false); // wait until we terminate it
...
int SSH::startRemoteControl(const QString &IPAddress, const QString &username, const QString &password) {
  ...
  executeSSHCommand("echo " + password + " | sudo -S /" + mainDirectoryName() + "/Linux/project/webots/remote_control/remote_control "
...
int SSH::uninstall(const QString &IPAddress, const QString &username, const QString &password) {
  ...
  if (openSSHSession(IPAddress, username, password) == -1)
    return -1;
  executeSSHCommand("ls /" + mainDirectoryName() + "/Linux/project/webots/backup/rc.local_original >/dev/null 2>&1 && echo 1 || echo 0", false);
  if (mStdout[0] == '1') // Restore rc.local
    executeSSHCommand("echo " + password + " | sudo -S cp /" + mainDirectoryName() + "/Linux/project/webots/backup/rc.local_original /etc/rc.local");
  executeSSHCommand("rm -rf /" + mainDirectoryName() + "/Linux/project/webots");
  closeSSHSession();
