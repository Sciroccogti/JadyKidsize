...
extern "C" {
#ifdef CROSSCOMPILATION
  const char *wbu_system_short_path(const char * path) { return path; }
#else
...
RobotisOp2MotionManager::RobotisOp2MotionManager(webots::Robot *robot) :
  mRobot(robot),
...
void RobotisOp2MotionManager::achieveTarget(int timeToAchieveTarget) {
  ...
  for (int i = 0; i < DMM_NMOTORS; i++) {
    if (mPositionSensors[i]->getSamplingPeriod() <= 0) {
      cerr << "The position feedback of the position sensor "<<  motorNames[i] << "S is not enabled. RobotisOp2MotionManager need to read the position of all position sensors. The position will be automatically enable."<< endl;
